...
class BlogPost(db.Model):
    ...
    author_id: Mapped[int] = mapped_column(db.Integer, db.ForeignKey("users.id"))
...
def register():
    ...
    if form.validate_on_submit():
        try:
            ...
            flash("This email already exists, log in please")
        finally:
...
def login():
    ...
    if form.validate_on_submit():
        try:
            ...
            correct_password = check_password_hash(user.password, form.password.data)
            if correct_password:
                login_user(user)
                return redirect(url_for('get_all_posts'))
            else:
            ...
            flash("Email not found", category="email_error")
...
def user_required(f):
    ...
    def decorator_function(*args, **kwargs):
        if current_user.is_authenticated:
            ...
            flash("Please login before view or commenting on the posts")
...
def edit_post(post_id):
    ...
    if edit_form.validate_on_submit():
        ...
        db.session.commit()
        return redirect(url_for("show_post", post_id=post.id, author_name=post.author.name))
